...
const index_1 = __importDefault(require("prisma/client/index"));
...
class AuthController {
    constructor() {
        this.register = async (req, res) => {
            try {
                ...
                const isAlreadyRegistered = await index_1.default.user.findUnique({
        ...
        this.login = async (req, res) => {
            try {
                ...
                await index_1.default.user.update({ where: { id: user.id }, data: { token } });
                res.status(200).json({
                    status: 200,
                    data: { ...user, token },
        ...
        this.forgotPassword = async (req, res) => {
            try {
                ...
                const otpExpiress = new Date(Date.now() + 5 * 60 * 1000);
                await (0, mailer_1.sendOTPEmail)(auth.email, otp);
                const newOtp = await index_1.default.user.update({
                    where: {
                        email: auth.email,
                    },
                    data: {
        ...
        this.getProfile = async (req, res) => {
            try {
                ...
                const auth = await index_1.default.user.findFirst({
                    where: {
                        id: user.id,
                    },
                });
        ...
        this.editProfile = async (req, res) => {
            try {
                ...
                const Auth = await index_1.default.user.update({
                    where: {
                        id: jwtUser.id,
                    },
                    data: updateData,
    ...
    async resetPassword(req, res) {
        try {
            ...
            const hashedPassword = await bcryptjs_1.default.hash(auth.password, 10);
            const newPassword = await index_1.default.user.update({
                where: { id: user.id },
